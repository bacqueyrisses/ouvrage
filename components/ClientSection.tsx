...
export default function ClientSection({
  ...
  const [dailyDescription, setDailyDescription] =
    useState<string>(storedDescription);
  const [newDescription, setNewDescription] = useState<string>("");
  const newWordRef = useRef<string>("");
  const index = Math.round(Math.random() * (words.length - 1));

  useEffect(() => {
    if (!dailyWord) window.location.reload();
  }, []);
  ...
  const handleRandom = () => {
    ...
    const currentDescription = words
      .find((value: string[]) => value[0] === newWordRef.current)
      ?.at(1);
